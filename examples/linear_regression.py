...
def make_lin_reg_prior():
    ...
    prior = distrax.Independent(distrax.MultivariateNormalDiag(mu, sigma))
    return prior
...
class Workspace:
    def __init__(self, cfg):
        ...
        if self.cfg.designs.num_xi is not None:
            if self.cfg.designs.d is None:
                self.d = jnp.array([])
                self.xi = jrandom.uniform(
                    rng, shape=(self.cfg.designs.num_xi,), minval=-10, maxval=10
                )
                self.d_sim = self.xi
            else:
        ...
        if self.xi_scheduler == "None":
            ...
            self.schedule = optax.linear_schedule(
                self.xi_lr_init, self.xi_lr_end, transition_steps=self.training_steps
            )
            ...
            self.schedule = optax.exponential_decay(
                ...
                decay_rate=(self.xi_lr_end / self.xi_lr_init)
            ...
            def cosine_decay_multirestart_schedules(
                ...
                for i in range(restarts):
                    ...
                    d = decay_steps * (i + 1)
                    s = optax.cosine_decay_schedule(lr, decay_steps, alpha=alpha)
                    schedules.append(s)
                    boundaries.append(d)
    ...
    def run(self) -> Callable:
        ...
        design_max = 10.0
        scale_factor = float(
            jnp.max(jnp.array([jnp.abs(design_min), jnp.abs(design_max)]))
        )
        xi_params_max_norm = {}
        ...
        for step in range(self.training_steps):
            ...
            exp_like_log_probs = distrax.MultivariateNormalDiag(
                x_noiseless, noise
            ).log_prob(x)
            ...
            print(
                f"STEP: {step:5d}; Xi: {xi_params['xi']}; \
...
def main(cfg):
    ...
    if os.path.exists(fname):
        ...
        print(
            f"STEP: {workspace.step:5d}; Xi: {workspace.xi};\
