...
class ConditionalChain(Chain):
    ...
    def forward_and_log_det(
        ...
        x, log_det = self._bijectors[-1].forward_and_log_det(x, theta, xi)
        for bijector in reversed(self._bijectors[:-1]):
            x, ld = bijector.forward_and_log_det(x, theta, xi)
            log_det += ld
        return x, log_det
    ...
    def inverse_and_log_det(
        ...
        y, log_det = self._bijectors[0].inverse_and_log_det(y, theta, xi)
        for bijector in self._bijectors[1:]:
            y, ld = bijector.inverse_and_log_det(y, theta, xi)
            log_det += ld
        return y, log_det
