...
def plot_contour_prior_posterior(
    prior_samples, posterior_samples, true_theta, filename
):
    ...
    x_grid_prior, y_grid_prior = np.meshgrid(
        np.linspace(x_prior.min(), x_prior.max(), num=100),
    ...
    ax1.contour(
        x_grid_prior, y_grid_prior, density_prior, levels=levels_prior, cmap="viridis"
    )
    ...
    # Create a grid of x and y values
    x_grid_posterior, y_grid_posterior = np.meshgrid(
        np.linspace(x_posterior.min(), x_posterior.max(), num=100),
        np.linspace(y_posterior.min(), y_posterior.max(), num=100),
    )
    z_grid_posterior = kde_posterior(
        np.vstack([x_grid_posterior.ravel(), y_grid_posterior.ravel()])
    )
    ...
    levels_posterior = np.linspace(
        density_posterior.min(), density_posterior.max(), num=10
    )
    ax2.contour(
        x_grid_posterior,
        y_grid_posterior,
        density_posterior,
        levels=levels_posterior,
        cmap="viridis",
    )
...
def plot_prior_posteriors(
    ...
    for i, posterior in enumerate(posteriors, start=1):
        ...
        kde_posterior = gaussian_kde(
            np.vstack([x_posterior, y_posterior]), bw_method=bandwidth
        )
        density_posterior = kde_posterior(np.vstack([x_posterior, y_posterior]))
...
def plot_prior_posterior(prior_samples, posterior_samples, true_theta, filename):
    ...
    ax1.hist(
        posterior_samples[:, 0], bins=30, alpha=0.5, color="orange", label="Posterior"
    )
    ...
    ax2.hist(
        posterior_samples[:, 1], bins=30, alpha=0.5, color="orange", label="Posterior"
    )
...
def sir_update(
    ...
    log_likelihoods = log_likelihood_fn.apply(
        likelihood_params, x_obs, prior_samples, xi
    )
